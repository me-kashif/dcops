...
def get_events_details(aci_cookie, apic_ip,req_date):
    ######### GET events instances ##############
    # GET EP details
    
    url = f'{apic_ip}/api/node/class/eventRecord.json?query-target-filter=and(not(wcard(eventRecord.dn,"__ui_")),wcard(eventRecord.created, "{req_date}"))&order-by=eventRecord.created|asc'
...
def save_to_csv(list_of_all_data):
    ...
    with open('events_history_data.csv', 'w', newline='')  as output_file:
...
def get_processed_data(get_events_details_result):

    fields = ['code', 'created','descr','severity','cause','affected']
    data = []

    for endpoints in get_events_details_result['imdata']:
        for endpoint_data in endpoints['eventRecord'].items():
            line_dict={}
            for field in fields:
    ...
    for row in data:
        ...
        temp_dict = {'creation_date':c_date,'creation_time':c_time,'events_code':code,'description':description,'severity':severity,'cause':cause,'affected':affected}
